#Dépendances propres à la fonction
import binascii
...
class GNSS:
	...
	fix_t=gngll_s[5]
	#fix_time=fix_t
	fix_time=str(int(float(fix_t)) // 10000) + 'h' + str((int(float(fix_t))%10000)//100) + 'min' + str((int(float(fix_t))%10000)%100) + 's'
        if gngll_s[2] == 'S':
            lat_d *= -1
        if gngll_s[4] == 'W':
            lon_d *= -1
        return(lat_d, lon_d, fix_time)

    def coordinates(self, debug=False):
        lat_d, lon_d, fix_time, debug_timeout = None, None, None, False
            ...
            if gngll_idx >= 0:
                ...
                if e_idx >= 0:
                    try:
                        ...
                        gngll_s = gngll.split(',')
                        lat_d, lon_d, fix_time = self._convert_coords(gngll_s)
                    except Exception:
                        pass
                    finally:
